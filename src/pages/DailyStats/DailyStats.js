...
import { toggleDailyStats, toggleConfig } from "../../redux/actions/actionsSlice";
import { Container, TitlePage, ContainerPages, ButtonPage, IconButton } from "../Config/ConfigStyles";
import { ChevronLeft, ChevronRight } from "react-feather";
import { pb } from "../../lib/pb";
import { formatPrice } from "../../utils/formatPrice";
import { getWeekRangeKeys, getMonthRangeKeys, fetchDailyStatsInRange, aggregateDailyDocs } from "../../utils/statsRanges"; // ajustá rutas
...
export default function DailyStats() {
  ...
  const goBackToConfig = () => { dispatch(toggleDailyStats(false)); dispatch(toggleConfig(true)); };
  ...
  if (loading) {
    return (
      <Container>
        <TitlePage><a>Daily Stats</a></TitlePage>
        <ContainerPages><ButtonPage><div style={{padding:16, textAlign:"center", width:"100%"}}>Cargando…</div></ButtonPage></ContainerPages>
      </Container>
    );
  }

  const rowLine  = { display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, width:"100%" };
  const rowLabel = { opacity:.7 };
  ...
  const Tabs = (
    <div style={{display:"flex", gap:8, margin:"6px 0 10px"}}>
      {["days","week","month"].map(k => (
        <button
          key={k}
          onClick={() => { setDetail(null); setView(k); }}
          style={{
            padding:"6px 10px", borderRadius:10, border:"1px solid #ddd", cursor:"pointer",
            background: view===k ? "#111" : "#fff", color: view===k ? "#fff" : "#111"
          }}
        >
      ...
      {(view==="week" || view==="month") && (
        <div style={{marginLeft:"auto", display:"flex", gap:8}}>
          <button
            onClick={()=>setCursor(new Date(cursor.getFullYear(), cursor.getMonth(), cursor.getDate() - (view==="week"?7:30)))}
      ...
      <Container>
        ...
        <ContainerPages>
          ...
          {rows.map((d) => {
            ...
            return (
              <ButtonPage key={d.id} onClick={() => setDetail(d)} style={{ display:"flex", alignItems:"center", gap:12, width:"100%" }}>
                <div style={{ display:"grid", gap:8, flex:1, minWidth:0 }}>
                  <div style={rowLine}><span style={rowLabel}>Fecha</span><strong style={rowValue}>{formatPrettyDate(fecha)}</strong></div>
                  <div style={rowLine}><span style={rowLabel}>Cantidad de pedidos</span><strong style={rowValue}>{pedidos}</strong></div>
                  <div style={rowLine}><span style={rowLabel}>Total</span><strong style={rowValue}>{formatPrice(total)}</strong></div>
                </div>
                <span style={{flexShrink:0}}><ChevronRight/></span>
              </ButtonPage>
            );
          })}
        </ContainerPages>
...
if (view==="week" && weekly?.agg && !detail) {
  ...
  return (
    <Container>
      <TitlePage><a>Daily Stats</a></TitlePage>
      <ContainerPages>
        <ButtonPage onClick={goBackToConfig}><IconButton><ChevronLeft/></IconButton><a>Volver a Config</a></ButtonPage>
        {Tabs}
        ...
        >
          <div style={{ display:"grid", gap:8, flex:1, minWidth:0 }}>
            <div style={rowLine}><span style={rowLabel}>Periodo</span><strong style={rowValue}>{periodLabel}</strong></div>
            <div style={rowLine}><span style={rowLabel}>Cantidad de pedidos</span><strong style={rowValue}>{agg.ordersCount}</strong></div>
            <div style={rowLine}><span style={rowLabel}>Total</span><strong style={rowValue}>{formatPrice(agg.revenue)}</strong></div>
          </div>
          <span style={{flexShrink:0}}><ChevronRight/></span>
        </ButtonPage>
      </ContainerPages>
    ...
    <Container>
      ...
      <ContainerPages>
        ...
        >
          <div style={{ display:"grid", gap:8, flex:1, minWidth:0 }}>
            <div style={rowLine}><span style={rowLabel}>Periodo</span><strong style={rowValue}>{periodLabel}</strong></div>
            <div style={rowLine}><span style={rowLabel}>Pedidos</span><strong style={rowValue}>{agg.ordersCount}</strong></div>
            <div style={rowLine}><span style={rowLabel}>Total</span><strong style={rowValue}>{formatPrice(agg.revenue)}</strong></div>
          </div>
          <span style={{flexShrink:0}}><ChevronRight/></span>
        </ButtonPage>
...
const cantDelivery= nz(byMode.delivery);
  ...
  return (
    <Container>
      <TitlePage><a>{detail.__summary ? "Detalle del resumen" : "Detalle del día"}</a></TitlePage>
      <ContainerPages>
        <ButtonPage onClick={()=>setDetail(null)}><IconButton><ChevronLeft/></IconButton><a>Volver</a></ButtonPage>
        ...
        <ButtonPage style={{ display:"flex", alignItems:"stretch", width:"100%" }}>
          <div style={{ display:"grid", gap:12, flex:1, minWidth:0 }}>
            ...
            <div style={{display:"grid", gap:6, width:"100%"}}>
              ...
              {itemsList.map(it => (
                <div key={it.name} style={{ ...rowLine, paddingLeft:12 }}>
                  <span style={{ fontWeight:400, opacity:.85 }}>{it.name}</span>
            ...
            {/* Take away / Delivery */}
            <div style={{display:"grid", gap:6, width:"100%"}}>
              <div style={rowLine}><span style={rowLabel}>Take away</span><strong style={rowValue}>{cantRetiro}</strong></div>
              <div style={rowLine}><span style={rowLabel}>Delivery</span><strong style={rowValue}>{cantDelivery}</strong></div>
            </div>

            {/* Métodos de pago */}
            <div style={{display:"grid", gap:6, width:"100%"}}>
              <div style={rowLine}><span style={rowLabel}>Efectivo</span><strong style={rowValue}>{formatPrice(montoEf)}</strong></div>
              <div style={rowLine}><span style={rowLabel}>Transferencia</span><strong style={rowValue}>{formatPrice(montoTr)}</strong></div>
            </div>

            <hr style={{ opacity:.15, border:"none", height:1, background:"#000" }} />
