...
export const ContainerHome = styled.main`
  ...
  padding: calc(${NAV_H}px + 0px) 0px
           ${({ $hasCart }) =>
             $hasCart
               ? `calc(${CART_H}px + 0px + env(safe-area-inset-bottom))`
               : `0px`}
           0px;
  background: #f7f8fa;
`;
...
export const CartBar = styled.div`
  ...
  ${p => p.$bump && css`animation: ${glow} .6s ease;`}
`;
...
export const CartFillFX = styled.div`
  ...
  animation: ${fillSwipe} .6s ease forwards;
  pointer-events: none;
 `;
...
export const CartCount = styled.div`
  ...
  ${p => p.$bump && css`animation: ${bump} .5s ease;`}
`;
