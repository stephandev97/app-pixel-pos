...
export const SidebarItem = styled.button.attrs(({ icon, label }) => ({
  ...
  &:hover { background: #f8f9fa; color: #111; }
`;
...
export const SidebarFooter = styled.div`
  ...
  .primary {
    ...
    background: #111; color: #fff; border: 0; padding: 10px 8px;
...
export const Content = styled.main`
  ...
  + .bottom-nav {
    ...
    height: 66px; background: #fff; border-top: 1px solid #e5e7eb;
    display: grid; grid-template-columns: repeat(3, 1fr);
    align-items: center; z-index: 160;

    button {
      background: none; border: 0; display: flex; flex-direction: column;
      align-items: center; gap: 4px; color: #6b7280; font-size: 12px;
    }
    button.active { color: #111; }
  }
  @media (min-width: 768px) {
    + .bottom-nav { display: none; } /* ocultar barra inferior en desktop */
  }
`;