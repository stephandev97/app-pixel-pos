...
const Checkout = () => {
    ...
    const cantidad = cartItems.reduce((acc, item) => {return (acc += item.quantity)}, 0)
    ...
    const price = cartItems.reduce((acc, item) => {
      ...
      return (
        <>
          <Dialog slots={{transition: Transition}} open={!show} fullScreen onClose={() => dispatch(toggleHiddenCart(true))}>
            <ContainerCheckout>
...
const Checkout2 = () => {
    ...
    const price = cartItems.reduce((acc, item) => {
      ...
      return (
        <>
        {show ?  
        <ContainerCheckout style={{'transform': 'translateX(490px)'}}>
          <ProductsCheckout cartItems={cartItems} price={price}/>
