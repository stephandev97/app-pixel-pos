...
export async function getDailyStatRecord({ dateKey }) {
  try {
    ...
    try {
      return await pb.collection('daily_stats')
...
function mergeNumericMap(prev = {}, delta = {}) {
  const out = { ...(prev || {}) };
  for (const k of Object.keys(delta || {})) {
    out[k] = (Number(out[k]) || 0) + (Number(delta[k]) || 0);
  }
  return out;
}

function buildMethodDeltas({ paymentMethod, revenueAdd = 0, paidAdd = null }) {
  const key = paymentMethod || 'desconocido';
  return {
    revDelta:  { [key]: Number(revenueAdd || 0) },
    ordDelta:  { [key]: 1 },
    paidDelta: paidAdd != null ? { [key]: Number(paidAdd || 0) } : null,
  };
}
...
// Si tu `day` es TEXT YYYY-MM-DD, esto alcanza:
...
async function getDailyStatByDayKey(dayKey) {
  try {
    return await pb.collection('daily_stats').getFirstListItem(`day="${dayKey}"`);
  } catch (e) {
...
export async function rebuildDailyStatsFromOrders(day) {
  ...
  const itemsCount = {}, revenueByMethod = {};
  ...
  const coll = pb.collection('daily_stats');
  let doc;
  try { doc = await coll.getFirstListItem(`day = "${day}"`); }
  catch { doc = await coll.create({ day, revenue: 0, ordersCount: 0, itemsCount: {}, revenueByMethod: {} }); }
...
export function itemsCountFrom(items = []) {
  ...
  const pretty = (it) => {
    ...
    // (fallback por si en algún flujo no llegó "sabores")
      ...
      const [base, slug] = it.sku.split("__");
      const nice = slug
        .split("-")
...
function buildItemKey(it = {}) {
  // Si usás variantes/sabores, podés combinar:
  // p.ej. "Helado 1/4 kg / Dulce de Leche"
  const base =
    it.name ??
    it.label ??
    it.title ??
  ...
  // Si querés incluir variante/tamaño/sabor en la clave:
  const variant =
    it.variant ?? it.size ?? it.tamano ?? (Array.isArray(it.options) ? it.options.join(", ") : null);

  const pretty = [base, variant].filter(Boolean).join(" / ");

  // Si no hay nada, recién ahí caemos al id
  const raw = pretty || it.productId || it.id || "item";
...
export async function upsertDailyStatsJsonSmart({
  ...
  let doc = null;
  try { doc = await coll.getFirstListItem(`day="${dayStr}"`); }
  ...
  let next = {
    ...
    ordersByMethod:  mergeCounts(
      ...
      paidKeys.length
        ...
        : (m ? { [m]: Math.abs(addOrders || 1) } : {}),
      sign
    ),
    ordersByMode:    mergeCounts(prev.ordersByMode, mode ? { [mode]: Math.abs(addOrders||1) } : {}, sign),
    deliveryByAddress: (() => {
...
/* ====== helpers ====== */
function toDayString(d){ const dt=d instanceof Date?d:new Date(d); return `${dt.getFullYear()}-${String(dt.getMonth()+1).padStart(2,'0')}-${String(dt.getDate()).padStart(2,'0')}`; }
function safeNumber(n,def=0){ const v=Number(n); return Number.isFinite(v)?v:def; }
function safeJson(v){ return (v && typeof v==='object' && !Array.isArray(v))?{...v}:{}; }
function mergeCounts(base={}, delta={}, sign=+1){ const out={...safeJson(base)}; for(const k of Object.keys(delta||{})){ out[k]=safeNumber(out[k])+sign*safeNumber(delta[k]); } return out; }
function clamp0(n){ return n<0?0:n; }
function clampCounts(obj={}, eps=0){ const out={}; for(const [k,v] of Object.entries(obj)) out[k]= (v < eps ? 0 : v); return out; }
function pruneZeroKeys(obj={}, eps=0){ const out={}; for(const [k,v] of Object.entries(obj)) if (Math.abs(Number(v)||0) > eps) out[k]=v; return out; }
function isEmptyMap(obj){ return !obj || Object.keys(obj).length === 0; }
function isDailyEmpty(n){
  return safeNumber(n.revenue)===0 &&
...
function normalizeMethod(m){ if(!m) return null; const s=String(m).toLowerCase(); if(s.startsWith('efec')) return 'efectivo'; if(s.startsWith('trans')) return 'transferencia'; return 'otro'; }
...
export async function upsertCustomerFromOrder({
  address, phone, name, total = 0, businessDate, sign = +1,
}) {
...
export function detectFulfillment(direccion, checkFlag = null) {
  ...
  const pickupKeywords = [
    "retiro","retira","retirar","pickup","pick up","take away","takeaway",
    "local","en local","mostrador","paso","pasa","voy"
  ];
