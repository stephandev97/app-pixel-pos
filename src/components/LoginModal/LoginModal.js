...
import { setShowLoginModal, setIsAdmin, toggleDailyStats, toggleConfig } from "../../redux/actions/actionsSlice";
      ...
      <DialogContent>
        <form onSubmit={handleSubmit} style={{display:"grid", gap:12}}>
          ...
          <input
            ...
            style={{padding:10, borderRadius:10, border: error ? "2px solid #e53935" : "1px solid #ccc"}}
            autoFocus
          />
          {error && <small style={{color:"#e53935"}}>{error}</small>}
          <button type="submit" style={{padding:"10px 14px", borderRadius:10, border:"none", cursor:"pointer"}}>
            Entrar
          </button>
        </form>
      </DialogContent>
